#!/bin/bash
# ZIVPN Final Fix: Added User Count to Dashboard
set -euo pipefail

# Python Web Script ကို အသစ်ရေးသားခြင်း
...
# Service ပြန်စတင်ခြင်း
systemctl restart zivpn-web

IP_ADDR=$(curl -s ifconfig.me || hostname -I | awk '{print $1}')
echo -e "\n\e[1;32m✅ User Count ပါဝင်အောင် ပြင်ဆင်ပြီးပါပြီ Bro!\e[0m"
