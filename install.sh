...
// Session မရှိလျှင် Login Form ပြမည်
...
<body>
    <div class="login-box">
        ...
        <form method="POST">
            <input type="text" name="user" placeholder="Admin User" required>
...
// ဒီအောက်ကအပိုင်းက Login ဝင်ပြီးမှ မြင်ရမှာပါ
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .nav { background: #1a73e8; color: white; padding: 15px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
...
<body>
    <div class="nav">
        ...
        <a href="?action=logout" style="color: white; text-decoration: none; font-weight: bold; font-size: 13px; padding: 5px 10px; border: 1px solid white; border-radius: 4px;">Logout</a>
    </div>
    <div class="container">
        <div class="card">
            ...
            <form method="POST">
                ...
                <button type="submit" name="create" class="btn-create">CREATE NOW</button>
            </form>
        </div>
    </div>
</body>
...
echo "------------------------------------------"
echo "ZiVPN Panel (Session Secure) Update Completed!"
echo "Link: http://$MYIP"
echo "------------------------------------------"
